...
const App = () => {
  ...
  const [groupActor, setGroupActor] = useState(null);
  ...
  const createGroup = async (name, members, amount) => {
    if (groupActor) {
      const memberObjects = members.map(m => ({ principal: m.principal, name: m.name }));
